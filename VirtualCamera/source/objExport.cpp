...
)
{
	...
	for ( ; !polyIter.isDone(); polyIter.next() )
	{
        ...
        //
		//===========modified by bubble, set the polygon to triangle===================

		int indexOff = 1;
		int polyVertexCount = polyIter.polygonVertexCount();
		for ( int vtx=0; vtx<polyVertexCount-2; vtx++ ) {
			//重建三角形索引
			fprintf(fp,"f");
			//1.the first pt of current triangle
			//	fprintf(fp," %d", polyIter.vertexIndex( 0 ) +1); 
			fprintf(fp," %d", polyIter.vertexIndex( 0 ) +1 +voff); 
			bool noUV = true;
			if ( fnMesh.numUVs() > 0 ) {
				int uvIndex;
				...
				// anything
				//
				if ( polyIter.getUVIndex(0,uvIndex) ) {
					//fprintf(fp,"/%d",uvIndex+1);
			...
			if ( (normals) && (fnMesh.numNormals() > 0) ) {
				...
				//fprintf(fp,"/%d",polyIter.normalIndex( 0 ) +1); 
				fprintf(fp,"/%d",polyIter.normalIndex( 0 ) +1 +vnoff);
			}
			//2.the second pt of current triangle
			//fprintf(fp," %d", polyIter.vertexIndex( indexOff) +1);
			fprintf(fp," %d", polyIter.vertexIndex( indexOff) +1 +voff);
			noUV = true;
			if ( fnMesh.numUVs() > 0 ) {
				int uvIndex;
				// If the call to getUVIndex fails then there is no
				...
				// anything
				//
				if ( polyIter.getUVIndex(indexOff,uvIndex) ) {
					//fprintf(fp,"/%d",uvIndex+1 );  
					fprintf(fp,"/%d",uvIndex+1 +vtoff); 
					noUV = false;
				}
			...
			if ( (normals) && (fnMesh.numNormals() > 0) ) {
				...
				//fprintf(fp,"/%d",polyIter.normalIndex( indexOff ) +1);
				fprintf(fp,"/%d",polyIter.normalIndex( indexOff ) +1 +vnoff);
			}
			//3.the third pt of current triangle
			//fprintf(fp," %d", polyIter.vertexIndex( indexOff + 1) +1);
			fprintf(fp," %d", polyIter.vertexIndex( indexOff + 1) +1 +voff);
			noUV = true;
			if ( fnMesh.numUVs() > 0 ) {
				int uvIndex;
				// If the call to getUVIndex fails then there is no
				...
				// anything
				//
				if ( polyIter.getUVIndex(indexOff + 1,uvIndex) ) {
					//fprintf(fp,"/%d",uvIndex+1);		
					fprintf(fp,"/%d",uvIndex+1 +vtoff);
					noUV = false;
				}
			...
			if ( (normals) && (fnMesh.numNormals() > 0) ) {
				...
				//fprintf(fp,"/%d",polyIter.normalIndex( indexOff + 1 ) +1 ); 
				fprintf(fp,"/%d",polyIter.normalIndex( indexOff + 1 ) +1 +vnoff);
			}
			fprintf(fp,"\n");
			indexOff++;
		}
		...
		if (indexOff >= polyVertexCount ) {
			fprintf(stderr,"Failure in  rebuildTriangles.\n");
			return MS::kFailure;
		}
		...
		//
		//MPointArray tempPtArray ;
		//MIntArray tempVtxList ;
		////get the vertices and vertex positions of all triangles in the current face's triangulation
		//polyIter.getTriangles(tempPtArray, tempVtxList, space); 
		//// this vertex 
		//unsigned polyIndex = polyIter.index();
		//int pointNum = tempPtArray.length();
		//int ptIndexNum = tempVtxList.length();
		//int triangleNum = tempVtxList.length()/3 ; 
/*
		fprintf(fp,"pt index is ");
		int a[100];
		tempVtxList.get(a);
		int maxList = tempVtxList.length();
		for(int t = 0; t<maxList; t++)
		{
			fprintf(fp,"%d,",a[t]);
		}


		fprintf(fp, "this is the %d face", polyIndex);
		fprintf(fp,"the point number is %d", pointNum);
		fprintf(fp,"the triangle number is %d", triangleNum);
*/
		//get the vertices around the face, and made the map from object-relative vertex index to face-relative vertex index

		//MIntArray objRelativeVtx;
		//polyIter.getConnectedVertices(objRelativeVtx);

		//int facePtLength = objRelativeVtx.length();
		//int *ptObjIndexArray = new int[facePtLength];
		//objRelativeVtx.get(ptObjIndexArray);

		//// ptObjIndexArray stores the object-relative indices of the vertices surrounding the vertices of the current face.

		//	//fprintf(fp," %d", polyIter.vertexIndex( vtx ) +1 +voff);
		//int vtx = 0;
		//int *ptIndex = new int[ptIndexNum];
		//tempVtxList.get(ptIndex);

		//for(int triIndex = 0; triIndex < triangleNum; triIndex++  )//print all triangles out
		//{
		//	fprintf(fp,"f");
		//	for(int j = 0; j<3; j++)
		//	{
		//		vtx = ptIndex[triIndex*3+j];
		//		fprintf(fp," %d", vtx + 1 + voff);
		//		//then print the uv index and normal index
		//		//get the face-relative index of the vertex
		//		int vtxFace;
		//		for( vtxFace=0; vtxFace<facePtLength; vtxFace++ )
		//		{
		//			if(vtx == ptObjIndexArray[vtxFace])
		//			{
		//				break;
		//			}
		//		}
		...
		//			// anything
		//			//
		//			if ( polyIter.getUVIndex(vtxFace,uvIndex) ) {
		//				fprintf(fp,"/%d",uvIndex+1 +vtoff);
		//				noUV = false;
		//			}
		//		}
		...
		//				fprintf(fp,"/");
		//			}
		//			fprintf(fp,"/%d",polyIter.normalIndex( vtxFace ) +1 +vnoff);
		//		}
		//	}
		//	fprintf(fp,"\n");
		//}
